...
const EditRestaurantServiceModal = ({ isOpen, onClose, onServiceUpdated, service }) => {
  ...
  const parsearDetallesRestaurante = (detallesString) => {
    try {
      ...
      return {
        tipoEstablecimiento: detalles.tipo_establecimiento || 'Restaurante gourmet',
        estiloGastronomico: detalles.estilo_gastronomico || 'Internacional',
        capacidad: detalles.capacidad || '',
        
        // Servicios
        comidaEnMesa: detalles.servicios?.comida_en_mesa || false,
        paraLlevar: detalles.servicios?.para_llevar || false,
        domicilio: detalles.servicios?.domicilio || false,
        buffet: detalles.servicios?.buffet || false,
        catering: detalles.servicios?.catering || false,
        
        // Horarios
        desayuno: detalles.horarios?.desayuno || false,
        almuerzo: detalles.horarios?.almuerzo || false,
        cena: detalles.horarios?.cena || false,
        veinticuatroHoras: detalles.horarios?.veinticuatro_horas || false,
        
        // Extras
        musicaEnVivo: detalles.extras?.musica_en_vivo || false,
        catasDeVinos: detalles.extras?.catas_de_vinos || false,
        maridajes: detalles.extras?.maridajes || false,
        menuDegustacion: detalles.extras?.menu_degustacion || false,
        
        // Promociones
        happyHour: detalles.promociones?.happy_hour || false,
        descuentosPorGrupo: detalles.promociones?.descuentos_por_grupo || false,
        menu: detalles.promociones?.menu || false,
        
        // Servicios adicionales
        piscina: detalles.servicios_adicionales?.piscina || false,
        parqueadero: detalles.servicios_adicionales?.parqueadero || false,
        petFriendly: detalles.servicios_adicionales?.pet_friendly || false,
        roomService: detalles.servicios_adicionales?.room_service || false,
        ascensor: detalles.servicios_adicionales?.ascensor || false,
        plantaEnergia: detalles.servicios_adicionales?.planta_energia || false,
        rampaDiscapacitados: detalles.servicios_adicionales?.rampa_discapacitados || false
      };
    } catch (error) {
  ...
  useEffect(() => {
    if (isOpen && service) {
      const detallesParsados = parsearDetallesRestaurante(service.detalles_del_servicio || '{}');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      Swal.fire({
        ...
        text: 'El servicio del restaurante ha sido actualizado exitosamente',
        icon: 'success',
      ...
      <div className="bg-white rounded-lg shadow-xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <div className="flex items-center space-x-3">
            ...
            <h2 className="text-xl font-bold text-gray-900">Editar Servicio de Restaurante</h2>
          </div>
          <button
            onClick={onClose}
